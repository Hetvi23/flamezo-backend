...
Run with:
    bench run-tests --app flamezo_backend --module flamezo_backend.flamezo.tests.test_subscription_tasks
"""
...
class TestApplyDeferredPlanChanges(unittest.TestCase):
    ...
    def setUp(self):
        self._sfx = frappe.generate_hash(length=6)
        from flamezo_backend.flamezo.tasks.subscription_tasks import apply_deferred_plan_changes
...
class TestSilverFeatureRenewals(unittest.TestCase):
    ...
    def setUp(self):
        self._sfx = frappe.generate_hash(length=6)
        from flamezo_backend.flamezo.tasks.subscription_tasks import process_silver_feature_renewals
