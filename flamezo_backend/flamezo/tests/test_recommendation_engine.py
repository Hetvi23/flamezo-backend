...
def _get_engine(mock_frappe=None):
    ...
    for key in list(sys.modules.keys()):
        if "flamezo_backend" in key and "recommendations" in key:
            del sys.modules[key]

    with patch.dict("sys.modules", {"frappe": mock_frappe}):
        with patch("flamezo_backend.flamezo.services.ai.base.get_openai_client"):
...
class TestLogCoOrderEvents(unittest.TestCase):
    ...
    def _run(self, order, mock_frappe=None):
        ...
        with patch.dict("sys.modules", {"frappe": mock_frappe, "frappe.utils": mock_frappe_utils}):
            from flamezo_backend.flamezo.tasks.recommendation_tasks import log_co_order_events
...
class TestEmbeddingCache(unittest.TestCase):
    ...
    def tearDown(self):
        self.patcher.stop()
        for key in list(sys.modules.keys()):
            if "recommendations" in key and "flamezo_backend" in key:
                del sys.modules[key]
