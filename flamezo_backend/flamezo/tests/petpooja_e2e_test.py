...
Run:
  ...
  bench execute flamezo_backend.flamezo.flamezo_backend.tests.petpooja_e2e_test.run_tests
"""
...
def make_petpooja_provider(rest):
    ...
    from flamezo_backend.flamezo.pos.petpooja import PetpoojaProvider
...
def test_9_callback_status_accepted(provider, order):
    ...
    for code in ["1", "2", "3"]:
        ...
        with patch("flamezo_backend.flamezo.api.realtime.notify_order_update"):
...
def test_14_callback_no_backwards_regression(provider, order):
    ...
    with patch("flamezo_backend.flamezo.api.realtime.notify_order_update") as mock_notify:
        provider.handle_callback(cb)
...
def test_27_duplicate_push_guard(provider, order):
    ...
    with patch("requests.post") as mock_post:
        from flamezo_backend.flamezo.pos.utils import push_order_to_pos_job
...
def test_28_utils_trigger_on_accepted(order):
    ...
    from flamezo_backend.flamezo.pos.utils import handle_order_update
...
def test_31_map_status_all_codes(provider):
    """T31: map_status returns correct Flamezo status for all Petpooja codes"""
...
def test_32_get_pos_provider_routing():
    ...
    from flamezo_backend.flamezo.pos.base import get_pos_provider
    from flamezo_backend.flamezo.pos.petpooja import PetpoojaProvider
...
def test_33_pos_gateway_sniff_petpooja():
    ...
    from flamezo_backend.flamezo.api.pos import _process_gateway_event
