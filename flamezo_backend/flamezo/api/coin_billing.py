...
def check_and_trigger_auto_recharge(restaurant, current_balance):
    ...
    frappe.enqueue(
        "flamezo_backend.flamezo.api.coin_billing.trigger_auto_recharge",
        restaurant=restaurant,
...
def trigger_auto_recharge(restaurant):
    ...
    try:
        ...
        payment = client.payment.createRecurring({
            ...
            "recurring": True,
            "description": f"Flamezo Autopay: Rs.{total_payable:.0f} for {restaurant}",
            "notes": {
                "restaurant": restaurant,
                "type": "auto_recharge",
                "base_amount": base_amount,
