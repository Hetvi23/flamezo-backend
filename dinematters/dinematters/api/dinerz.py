...
"""
DinerZ Consumer App APIs
Aggregated endpoints for the DinerZ multi-restaurant super-app.
...
def get_all_restaurants(latitude=None, longitude=None, search=None, city=None, page=1, limit=30):
	"""
	GET /api/method/dinematters.dinematters.api.dinerz.get_all_restaurants

	Returns all active DinerZ-enrolled restaurants for the discovery feed.
	...
	try:
		...
		cache_key = f"dinerz:restaurants:{lat_bucket}:{lon_bucket}:{search or ''}:{city or ''}:{page}:{limit}"
		...
		frappe.log_error(f"Error in dinerz.get_all_restaurants: {str(e)}")
...
def get_cross_restaurant_offers(city=None, page=1, limit=30):
	"""
	GET /api/method/dinematters.dinematters.api.dinerz.get_cross_restaurant_offers

	Returns active coupons/offers across all GOLD-plan DinerZ restaurants.
	...
	try:
		...
		cache_key = f"dinerz:offers:{city or 'all'}:{page}:{limit}"
		if frappe.session.user == "Guest":
			cached = frappe.cache().get_value(cache_key)
			if cached:
				return json.loads(cached)
		...
		frappe.log_error(f"Error in dinerz.get_cross_restaurant_offers: {str(e)}")
		return {"success": False, "error": {"code": "OFFERS_FETCH_ERROR", "message": str(e)}}


# ── 3. DinerZ Member Profile ──────────────────────────────────────────────────

@frappe.whitelist(allow_guest=True)
def get_dinerz_member(phone=None):
	"""
	GET /api/method/dinematters.dinematters.api.dinerz.get_dinerz_member

	Returns the DinerZ unified member profile for the authenticated customer.
	...
	try:
		...
		return {
			...
			"data": {
				...
				"dinerz_points_balance": flt(balance),
				"tier": tier,
				"next_tier": next_tier,
		...
		frappe.log_error(f"Error in dinerz.get_dinerz_member: {str(e)}")
		return {"success": False, "error": {"code": "MEMBER_FETCH_ERROR", "message": str(e)}}


# ── 4. DinerZ Points Ledger ───────────────────────────────────────────────────

@frappe.whitelist(allow_guest=True)
def get_points_ledger(phone=None, page=1, limit=20):
	"""
	GET /api/method/dinematters.dinematters.api.dinerz.get_points_ledger

	Returns the unified DinerZ points transaction history for the customer,
	across ALL restaurants. Same auth pattern as get_dinerz_member.
	...
	try:
		...
		formatted_entries = []
		for e in entries:
			restaurant_name = frappe.db.get_value("Restaurant", e.restaurant, "restaurant_name") if e.restaurant else "DinerZ"
		...
		frappe.log_error(f"Error in dinerz.get_points_ledger: {str(e)}")
		return {"success": False, "error": {"code": "LEDGER_FETCH_ERROR", "message": str(e)}}


# ── 5. Register DinerZ Member ─────────────────────────────────────────────────

@frappe.whitelist(allow_guest=True)
def register_dinerz_member(phone, full_name=None, city=None):
	"""
	POST /api/method/dinematters.dinematters.api.dinerz.register_dinerz_member

	Creates or updates a Customer record for DinerZ.
	...
	try:
		...
		frappe.log_error(f"Error in dinerz.register_dinerz_member: {str(e)}")
...
def get_restaurant_summary(restaurant_id):
	"""
	GET /api/method/dinematters.dinematters.api.dinerz.get_restaurant_summary

	Lightweight restaurant summary for DinerZ link previews, notifications,
	...
	"""
	try:
		cache_key = f"dinerz:restaurant_summary:{restaurant_id}"
		cached = frappe.cache().get_value(cache_key)
		if cached:
			return json.loads(cached)
		...
		frappe.log_error(f"Error in dinerz.get_restaurant_summary: {str(e)}")
